...
static Shift::SC_Ref<Shift::SR_Texture> g_FontsTextureKeepAlive = NULL;
static Shift::SR_Descriptor_DX12    g_FontsSRV = {};
...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx)
{
	...
	Shift::SR_GraphicsDevice_DX12* device = static_cast<Shift::SR_GraphicsDevice_DX12*>(Shift::SR_GraphicsDevice::GetDevice());

	const Shift::SR_DescriptorHeap_DX12& heap = device->GetGPUVisibleDescriptorHeap();
	ID3D12DescriptorHeap* descriptorHeap = heap.GetDescriptorHeap();
	ctx->SetDescriptorHeaps(1, &descriptorHeap);
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            {
                Shift::SR_Descriptor_DX12 descriptor = device->AllocateDescriptorRange(1, false, Shift::SR_GraphicsContext::GetCurrent());
				ctx->SetGraphicsRootDescriptorTable(1, descriptor.myGPUHandle);  

                // Apply Scissor, Bind texture, Draw
                const D3D12_RECT r = { (LONG)(pcmd->ClipRect.x - clip_off.x), (LONG)(pcmd->ClipRect.y - clip_off.y), (LONG)(pcmd->ClipRect.z - clip_off.x), (LONG)(pcmd->ClipRect.w - clip_off.y) };
                //ctx->SetGraphicsRootDescriptorTable(1, *(D3D12_GPU_DESCRIPTOR_HANDLE*)&pcmd->TextureId);

                Shift::SR_Texture_DX12* textureRef = static_cast<Shift::SR_Texture_DX12*>((Shift::SR_Texture*)pcmd->TextureId);
                if (textureRef)
					device->GetNativeDevice()->CopyDescriptorsSimple(1, descriptor.myCPUHandle, textureRef->GetDescriptor(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        Shift::SR_GraphicsDevice_DX12* device = Shift::SR_GraphicsDevice_DX12::GetDX12Device();

        Shift::SR_TextureBufferDesc texBufDesc;
        texBufDesc.myDimension = Shift::SR_Dimension_Texture2D;
        texBufDesc.myMips = 1;
        texBufDesc.myWidth = (float)width;
        texBufDesc.myHeight = (float)height;
        texBufDesc.myFormat = Shift::SR_Format_RGBA8_Unorm;
        Shift::SC_Ref<Shift::SR_TextureBuffer> texBuf = device->CreateTextureBuffer(texBufDesc, "ImGui/Fonts");

        ID3D12Resource* pTexture = texBuf->myDX12Resource;
        ...
        Shift::SR_TextureDesc texDesc;
        texDesc.myFormat = texBufDesc.myFormat;
        texDesc.myMipLevels = 1;
        g_FontsTextureKeepAlive = device->CreateTexture(texDesc, texBuf);
        g_FontsSRV = device->GetGPUVisibleDescriptorHeap().Allocate();
    }
    ...
    io.Fonts->TexID = (ImTextureID)g_FontsTextureKeepAlive.Get();
}
